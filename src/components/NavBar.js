...
const NavBar = () => {
  ...
  return (
    // relative - for fixed nav
    <header
      className="w-full px-32 py-8 font-medium flex items-center justify-between
    dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8
    "
    >
      {/* button is shown after 800 px  */}
      <button
      ...
      {/* desktop nav || nav gets hidden after 800px */}
      <div className="w-full flex justify-between items-center lg:hidden">
        ...
        <nav className=" flex items-center justify-center flex-wrap mt-5 ">
          ...
          <button
            ...
            className={`ml-3 items-center justify-center p-1 rounded-full
          ${
            mode === "light"
              ? "bg-dark text-light fill-light"
              : "bg-light text-dark fill-dark"
          };
          `}
          >
            {mode === "dark" ? (
      ...
      {/* mobile nav || from fixed -> to show nav in center */}
      {isOpen ? (
        <motion.div
          initial={{ scale: 0, opacity: 0, x: "-50%", y: "-50%" }}
          animate={{ scale: 1, opacity: 1 }}
          className="min-w-[70vw] flex flex-col justify-between items-center 
      fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 bg-dark/90 text-light dark:text-dark dark:bg-light/75 rounded-lg backdrop-blur-md py-32 "
        >
